...
public class JiduTaskInfoActivity extends AbsCreatActivity implements OnGetGeoCoderResultListener
{
    ...
    private void getallotData()
    {
        ...
        new SendDataTask()
        {
            ...
            protected void onPostExecute(Object result)
            {
                ...
                {
                    Toast.makeText(mContext, "服务器数据错误" + result, Toast.LENGTH_SHORT).show();
                    JiduTaskInfoActivity.this.finish();
                    return;
                }
                String[] strlatlng = res.get("LatitudeLongitude").toString().split(",");
                ((TextView) findViewById(R.id.text_time)).setText((res.get("AllotedDate") == null||res.get("AllotedDate").toString().toLowerCase().equals("null")) ? "" : res.get("AllotedDate").toString());
                ((TextView) findViewById(R.id.text_cuttime)).setText((res.get("CutOffTime") == null||res.get("CutOffTime").toString().toLowerCase().equals("null")) ? "" : res.get("CutOffTime").toString());
                ...
                mSearch.setOnGetGeoCodeResultListener(JiduTaskInfoActivity.this);
...
//				ImageUtil.getBitmapAsyn(NetApiUtil.ImgBaseUrl + res.get("Picture") + "", img_Photo);
        ...
        }.execute(new ParamData(ApiCode.GetEvaluateEventDetails, id, OverAllData.getLoginId()));
    }
    ...
    private void InputStep(List<HashMap<String, Object>> stepList)
    {
        ...
        for (int i = 0; i < stepList.size(); i++)
        {
            ...
            if (nextmap != null && nextmap.get("Type") != null && nextmap.get("Type").toString().equals("0"))//上报
            {
                view.findViewById(R.id.linear_jiaoban).setVisibility(View.GONE);
                if (i == 0)
                {
    ...
    public void onClick(View v)
    {
        ...
        switch (v.getId())
        {
            ...
            case R.id.bt_callback:
                ...
                isNeedRefresh = true;
                startActivity(new Intent(mContext, DothisTaskActivity.class).putExtra("lat", latlng.latitude).putExtra("isjidu", true).putExtra("lng", latlng.longitude));
                break;
    ...
    private void toComfirm()
    {
        ...
        new SendDataTask()
        {
            ...
            protected void onPostExecute(Object result)
            {
                ...
                {
                    Toast.makeText(mContext, res.get("Message").toString(), Toast.LENGTH_SHORT).show();
                    return;
                }
                super.onPostExecute(result);
            }
