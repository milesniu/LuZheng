...
public abstract class SendDataTask extends AsyncTask<ParamData, String, Object>
	{
		...
		protected Object doInBackground(ParamData... parm)
		{
			...
			// if (OverAllData.IMEI != null && (!OverAllData.IMEI.equals("")) &&
			// (!OverAllData.getLoginId().equals("")))
			// {
			// HttpGetUtil.httpUrlConnection(ApiCode.SaveClientOnline,
			// OverAllData.getLoginId(), OverAllData.IMEI);
			// }
			...
			case GetBubbleCount:
			case ReadNotice:
				return HttpGetUtil.httpUrlConnection(parm[0].getCode(),
						parm[0].getParms());
			...
			case AddEvaluateReceive:
				return HttpPostUtil.httpUrlConnection(parm[0].getCode(),
						parm[0].getParms()[0]);
			...
			case AddEvaluate:
				return HttpPostUtil.httpUrlConnection(parm[0].getCode(),
						parm[0].getParms()[0], parm[0].getParms()[1]);
			}
			return null;
		}
