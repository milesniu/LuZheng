...
//				img.setImageBitmap(((SoftReference<Bitmap>) item.get("bitmap")).get());
            ...
            case taskManger:
                ...
                ((TextView) view.findViewById(R.id.text_descrption)).setVisibility(View.GONE);//.setText(item.get("EventContent")+"");
                ((TextView) view.findViewById(R.id.text_status)).setText(item.get("State") + "");
                ((TextView) view.findViewById(R.id.text_time)).setText((item.get("CutOffTime") == null || item.get("CutOffTime").toString().toLowerCase().equals("null")) ? "" : item.get("CutOffTime") + "");//item.get("State")+"");
